...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Deprecated: set both --gen_layer_name and --rank_layer_name",
    )
    parser.add_argument('--gen_layer_name', type=str, default="layer2")
    parser.add_argument('--rank_layer_name', type=str, default="layer2")
    ...
    if args.layer_name is not None:
        args.gen_layer_name = args.layer_name
        args.rank_layer_name = args.layer_name
    ...
    if args.gen_layer_name not in gen_layer_sizes:
        parser.error(f"{args.gen_model} has no layer {args.gen_layer_name!r}")
    if args.rank_layer_name not in rank_layer_sizes:
        parser.error(f"{args.rank_model} has no layer {args.rank_layer_name!r}")
    n_pix_feat = gen_layer_sizes[args.gen_layer_name][-1]
    n_pix_rank = rank_layer_sizes[args.rank_layer_name][-1]
    n_pix_img = 224
    ...
    def score_generated_images_for_voxel(
        ...
        with torch.no_grad():
            for batch_start in range(0, len(image_records), batch_size):
                ...
                image_features = encoder(batch_tensor)[layer_name].double()
    ...
    rank_prf_stack = get_prf_stack(
        n_pix=n_pix_rank, which_prf_grid='default-log-polar'
    )
    ...
    for region in regions:
        ...
        for i, idx in enumerate(topk_indices):
            ...
            channel_used = np.arange(
                gen_layer_sizes[args.gen_layer_name][0], dtype=int
            )
            if weights.shape[1] != channel_used.size:
                raise ValueError(
                    f"Generation weights for voxel {voxel_id} contain "
                    f"{weights.shape[1]} channels, but "
                    f"{args.gen_model}/{args.gen_layer_name} requires "
                    f"{channel_used.size}"
                )
            features_m = best_features_m[voxel_id, channel_used].double()
            features_s = best_features_s[voxel_id, channel_used].double()
            ...
            def single_voxel_objective(image_input):
                ...
                image_features = image_features[args.gen_layer_name].double().squeeze(0)
            ...
            model_folder = (
                f"gen-{path_component(args.gen_model)}-{path_component(args.gen_layer_name)}"
                f"__rank-{path_component(args.rank_model)}-{path_component(args.rank_layer_name)}"
            )
            if args.contrast_constraint_method == 'max':
                    ...
                    json.dump(
                        {
                            ...
                            "generation_model": {
                                "name": args.gen_model,
                                "layer": args.gen_layer_name,
                            },
                            "ranking_model": {
                                "name": args.rank_model,
                                "layer": args.rank_layer_name,
                            },
                            "generation": {
            ...
            rank_prf_gaussian = torch.as_tensor(
                rank_prf_stack[:, :, prf_idx],
                device=device,
                dtype=torch.double,
            )
            ...
            while len(image_records) < args.num_seeds:
                gen_scores = score_generated_images_for_voxel(
                    ...
                    layer_name=args.gen_layer_name,
