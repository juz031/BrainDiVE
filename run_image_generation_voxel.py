...
for subject in all_subjects:
    ...
    for region in regions:
        ...
        # Get the indices of the top 5 r2 values within the region mask
        region_r2_values = r2_values[region_mask]
        topk_r2_values, topk_r2_indices = torch.topk(region_r2_values, k=min(5, region_r2_values.numel()))
        print(f"Top 5 r2 values: {topk_r2_values}")

        # # Create a mask of same shape as region_mask with True at top 5 r2 voxels
        # flat_region_mask = region_mask.flatten()
        # region_mask_indices = flat_region_mask.nonzero(as_tuple=True)[0]
        # topk_voxel_indices = region_mask_indices[topk_indices]

        # # Now create a mask the same size as the input mask with True at those 5 locations
        # topk_mask_flat = torch.zeros_like(flat_region_mask, dtype=torch.bool)
        # topk_mask_flat[topk_voxel_indices] = True
        # topk_mask = topk_mask_flat.view(region_mask.shape)
        ...
        for r2_value, r2_voxel in zip(topk_r2_values, topk_r2_indices):
            print(f"Starting S{subject} {region} Voxel {r2_voxel} with r2 value {r2_value:.4f}")
            current_folder = f"/user_data/junruz/BrainDiVE/S1_600e_single_voxel/test_step{num_steps}_scale{brain_guidance_scale}/{f"S{subject}"}/{region}/{r2_voxel}_{r2_value:.4f}"
            # try:
            os.makedirs(current_folder, exist_ok=True)
            offset = 0

            def singel_voxel_r2_objective(image_input):
                # Aim: Maximize the predicted mean response for the region
                image_featrues = backbone.encode_image(image_input)
                image_featrues = image_featrues/image_featrues.norm(dim=-1, keepdim=True)
                pred_voxels = brain_projector(image_featrues.float())
                pred_response = pred_voxels[region_mask]
                # pred_response = pred_response[r2_mask]
                print(f'voxel index: {r2_voxel}')
                return -pred_response[r2_voxel]

            pipe.brain_tweak = singel_voxel_r2_objective
