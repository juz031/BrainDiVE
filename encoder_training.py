...
if __name__ == "__main__":
    ...
    feature_extractor, _ = clip.load(model_name, device=device)

    del feature_extractor.transformer
    torch.cuda.empty_cache()
    feature_extractor.eval()
    projector = soft_quantizer_v3(num_higher_output=n_voxels, embed_dim=1024)
    ...
    for epoch in tqdm(range(start_epoch, args.epochs)):
        ...
        torch.save(projector.state_dict(), f'weights/S{args.subject_id[0]}_{model_name}_bs{args.batch_size}_e{args.epochs}_{args.optimizer_name}_lr{args.lr_init}_decay{args.lr_decay}_wd{args.weight_decay}.pth')
    ...
    noise_ceiling = loader.noise_ceiling[str(args.subject_id[0])] / 100
    plt.figure()
    ...
    plt.savefig(f'figure/S{args.subject_id[0]}_{model_name}_r2_vs_noise_ceiling_epochs{args.epochs}_bs{args.batch_size}_{args.optimizer_name}_lr{args.lr_init}_decay{args.lr_decay}_wd{args.weight_decay}.png', dpi=300)
    ...
    plt.savefig(f'figure/S{args.subject_id[0]}_{model_name}_r2_voxels_hist_epochs{args.epochs}_bs{args.batch_size}_{args.optimizer_name}_lr{args.lr_init}_decay{args.lr_decay}_wd{args.weight_decay}.png', dpi=300)
